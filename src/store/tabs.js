...
export const useTabsStore = defineStore(
    ...
    () => {
        ...
        function removeOtherTab() {
            tabArr.value = tabArr.value.filter(item => item.path === route.path || item.path === '/' || item.path === '/home')
            const cacheArr = useCachePagesStore().getCachePages().filter(item => tabArr.value.map(item2 => item2.name).includes(item))
